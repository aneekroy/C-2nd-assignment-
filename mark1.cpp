...
void student::get_data(student &s){
		...
		cout<<"enter name:";
		cin.ignore();
		cin.getline(s.name,100);//to incorporate spaces in  the string
	/*to check if a string contains digits or not*/	
		for(int h=0;h<100;h++){
			if(s.name[h]=='1' || s.name[h]=='2'||s.name[h]=='3'||s.name[h]=='4'||s.name[h]=='5'||s.name[h]=='6'||s.name	[h]=='7'||s.name[h]=='8'||s.name[h]=='9'){
				fg=1;
				cout<<"name contains digits enter correctlty"<<endl;
				break;}
		}
		if(fg!=1){
			cout<<"\n";
			...
			cin>>s.course;
			cout<<"enter marks and subject"<<endl;
			for(j=0;j<5;j++){
				cin>>s1>>m;
				if(m<0 || m>100){         //marks should not > 100 or <0
					cout<<"wrong marks entered"<<endl;break;}
				s.sub[j]=s1;
				s.marks[j]=m;
			}
		}
...
void student::display(student s){
		...
		for(int y=0;y<5;y++){
			cout<<(y+1)<<"\t\t"<<s.sub[y]<<"\t \t\t " << s.marks[y]<<endl;
		}
}		
...
void batch::display1(batch b){
		cout<<"NAME \t\t"<<"ROLL NO \t\t "<<"COURSE"<<endl;
		for(int i=0;i<count;i++){
			cout<<b.list[i].get_name(b.list[i])<<"\t\t"<<b.list[i].get_roll(b.list[i])<<"\t\t\t"<<b.list[i].get_course(b.list[i])<<endl;
		}
}
...
int main(void){
	...
	/*giving the user the choice to enter any of the following options*/
		cout<<"----------GIVE ANY OF THE FOLLOWING CHOICES------------------"<<endl;
		cout<<"1.ADMIT A NEW STUDENT"<<endl;
		cout<<"2.CHANGE MARKS OF A PARTICULAR STUDENT"<<endl;
		cout<<"3.DISPLAY THE MARKSHEET OF ALL THE STUDENTS"<<endl;
		...
		if(choice==6)
			...
			switch(choice){
				...
				case 4:cout<<"give the roll no of students whose marks is to be displayed"<<endl;
					...
					break;	
				case 5:cout<<"the no of students is taken admission is:"<<b.get_count(b)<<endl;//to display the no of students-													  aking admission
					break;
